...
The repository now contains the schema-1.8 engineering v21 artifact for `PM-ABC-001`, bound to Phone Dino 0.7.4 runtime digest `sha256:db36e4c6a083714415218e623485f89e7a1fa2ae89dcbc46f5cabfe93d787a93` and artifact digest `sha256:1535ee521488fd6f252e11baacd40f7c67ab201fae8ccf8896385724d6863139`. It retains the real DINOv2/MobileSAM evidence pipeline and fail-closed multi-profile selection. Whole-subject dimensions use qualified ChArUco geometry; retained candidates prefer ChArUco and may use an operator-confirmed Golden proportional estimate only when Current ChArUco is absent. QR and visible rulers are not measurement inputs. Subject-mask, ROI, scorer-input, and recipe-profile digests remain pinned across both services. The compiled Golden ID is `GOLDEN-ACTIVE-V19-4FBB2AEE`; the active runtime/artifact identity is v21. It remains `ENGINEERING_AUTO` and `productionAuthorized=false`; this is not a signed production artifact.

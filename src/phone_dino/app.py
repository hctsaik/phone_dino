...
from .contracts import AnalyzeObservation, AnalyzeRequest
...
def create_app(settings: Settings | None = None, production_analyzer: ProductionAnalyzer | None = None) -> FastAPI:
    ...
    async def readyz() -> dict[str, object]:
        ...
        body = {"status": "ready" if ready else "not_ready", "simulation": resolved.fixture_enabled}
        if not token_ready:
    ...
    async def analyze(
        ...
        if request.simulation is not resolved.fixture_enabled:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="SIMULATION_MODE_MISMATCH")
        if not resolved.fixture_enabled and resolved.artifact_package_digest is not None:
